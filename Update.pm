...
### Nope
#
#my $HMM_LIB = "/home/rschaeff/data/pfam/Pfam-A.hmm";
...
sub process_side_load_domains_to_ecod { 
	...
	if ($size > 0) { 
		...
		YX_DOMAIN:
		foreach my $yx_domain_node ($yx_domain_xml_doc->findnodes($domain_XPath)) { 
...
#Domains
	my (%new_domain_seen, %new_domain_parent, %new_domain_range, %new_domain_uid, %new_domain_unhash, %new_domain_parent_uid, %new_domain_fid, %new_domain_type, %new_domain_assembly);
	my %new_domain_da;
...
sub apply_merge_v2 { 
	...
	add_new_mod_old_ecod_nodes($old_index, $new_index, $new_merge_index, $mod_merge_index, $obsolete_merge_index, $old_ecod_xml_doc);
...
#		}
	...
	foreach my $new_h (@{$$new_merge_index{h}}) { 
		...
		my $parent_x_group_node = exists $$old_index{x}{$parent_x_id} ? $$old_index{x}{$parent_x_id} : die "ERROR! Could not find parent X-group node $parent_x_id for H-group $new_h_id\n";
	...
	foreach my $new_f (@{$$new_merge_index{f}}) { 
		...
		my $parent_h_group_node = exists $$old_index{x}{$parent_h_id}  ?
	...
	foreach my $mod_f (@{$$mod_merge_index{f}}) { 
		...
		if ($$mod_f{f_shift}) { 
			...
			my $parent_h_group_node = exists $$old_index{h}{$parent_h_id} ? 
				$$old_index{h}{$parent_h_id} : 
	...
	foreach my $new_d (@{$$new_merge_index{d}}) { 
		...
		my $new_d_node = exists $$new_index{d}{$new_d_uid} ? $$new_index{d}{$new_d_uid} :
		...
		if (isDomainAssembly($new_d_node)) { 
			...
			my $parent_f_node = exists $$old_index{f}{$parent_f_id} ? $$old_index{f}{parent_f_id} : 
	...
	foreach my $mod_d (@{$$mod_merge_index{d}}) { 
		...
		if ($$mod_d{derived_shift}) { 
			...
			my $mod_d_node = exists $$old_index{c}{$mod_d_uid} ? $$old_index{d}{$mod_d_uid} : 
	...
	foreach my $mod_node (find_modification_merge_nodes($merge_xml_doc)) { 
		...
		if ($ecod_node_type eq 'x_group') { 
			...
			if ($modification_type eq 'derived_shift') { 
				...
				print "WARNING! modification_type $modification_type not supported for node type $ecod_node_type, skipping...\n";
	...
	foreach my $domain_node ($ecod_xml_doc->findnodes($domain_XPath)) { 
		...
		if (!$domain_node->exists('seqid_range')) { 
			if (!isMultiChainDomain($domain_node)) { 
				my ($seqid_range, $pdb_range) = convert_manual_range_to_seqid_range($manual_range, $pdb_id, $chain_id);
				...
				my ($mc_seqid_range, $mc_pdb_range) = convert_mc_manual_range_to_seqid_range($manual_range, $pdb_id);
...
sub find_overlaps { 
	...
	foreach my $chain_node ($chainwise_xml_doc->findnodes($chain_xpath)->get_nodelist() ) { 
		...
		foreach my $uid (keys %conflicts) { 
			...
			if ($conflicts{$uid} > 0  && $range_type{$uid} ne "manual" && $range_type{$uid} ne "implicit" && $range_type{$uid} ne 'curated') { 
